...
(function ($, _) {
    ...
    Quinn.prototype.setTentativeValue = function (newValue, animate, silent) {
        ...
        if (this.model.values.length > 1 && _.isNumber(newValue)) {
            ...
            //dev: update pointer from a clone
            var clonedElem = _.clone(this.model.values[this.activeHandle])
            clonedElem.value = newValue;

            scalar   = this.model.sanitizeValue(clonedElem);
            ...
            if (typeof prevScalar == "object" && prevScalar) {
                prevScalar = prevScalar.value;
            }
            if (typeof nextScalar == "object" && nextScalar) {
                nextScalar = nextScalar.value;
            }
            ...
            clonedElem.value = scalar;

            newValue[this.activeHandle] = clonedElem;
        }
    ...
    Quinn.prototype.rightClick = function (event) {
        // Remove the events which were bound in `startDrag`.
        this.activateHandleWithEvent(event);
        console.log("right click", this.activeHandle)
    ...
    Quinn.prototype.activateHandleWithEvent = function (event) {
        ...
        closestValue = _.min(this.model.values.map(function(x){return x.value}), function (handleValue) {
            return Math.abs(handleValue - value);
        });

        handleIndex = _.indexOf(this.model.values.map(function(x){return x.value}), closestValue);
    ...
    Model.prototype.setValue = function (newValue, strict) {
        ...
        for (i = 0, length = newValue.length; i < length; i++) {
            var scalar = this.sanitizeValue(newValue[i], strict);
            var objectValue = this.toObjectValue(newValue[i]);
            objectValue.value = scalar;
            newValue[i] = objectValue;
        }
    ...
    Model.prototype.toObjectValue = function (value) {
    ...
    Quinn.Renderer.prototype.render = function () {
        ...
        for (i = 0, length = this.model.values.length; i < length; i++) {
            var numValue = typeof this.model.values[i] == "object" && this.model.values[i] ? 
                this.model.values[i].value : this.model.values[i];
            var titleValue = typeof this.model.values[i] == "object" && this.model.values[i] ? 
                (this.model.values[i].title || '') : '';
            this.handles[i] = $('<div class="handle" tabindex="0" role="slider"><div class="pointer-title"></div><div class="space">&nbsp;</div><div class="value"></div></div>')
                .attr('aria-valuemin', this.model.minimum)
                .attr('aria-valuemax', this.model.maximum)
                .attr('aria-valuenow', numValue)
                .attr('aria-valuetitle', titleValue);

            if (typeof this.options.formatText == 'function') {
                this.handles[i].find('div.value').text(this.options.formatText(numValue));
            } else {
                this.handles[i].find('div.value').text(numValue);
            }
            this.handles[i].find('div.pointer-title').text(titleValue);
    ...
    Quinn.Renderer.prototype.redraw = function (animate) {
        ...
        _.each(this.model.values, function (value, i) {
            ...
            // dev
            var valueTitle = typeof value == "object" && value ? 
                (value.title || "") : "";
            ...
            handle.attr('aria-valuenow', value).attr('aria-valuetitle', valueTitle);

            if (typeof self.options.formatText == 'function') {
                handle.find('div.value').text(self.options.formatText(value));
            } else {
                handle.find('div.value').text(value);
            }
            ...
            if (animate && self.options.effects) {
                ...
                self.redrawDeltaBar(value.value, handle);
            }
        });
    ...
    $.fn.quinn = function (options) {
        return $.each(this, function () { this.superslide = new Quinn($(this), options); });
    };
